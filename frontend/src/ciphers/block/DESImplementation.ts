...
export function DESEncrypt(message: string, key: string): string {
  ...
  // Encrypts 8-byte blocks and pushes it to the blocks array.
  ...
  // Encrypts each block and appends it to the encryptedBlocks array
  let encryptedBlocks = [];
...
export function DESDecrypt(hexstring: string, key: string): string {
  ...
  // Appends 64-bit blocks to the blocks array from the binary string input.
  var blocks = [];
  for (var i = 0; i < binstring.length; i += 64) {
    blocks[i / 64] = binstring.slice(i, i + 64);
  }

  // Decrypts every block and appends to the decrypted block array
  var decryptedBlocks = [];
  for (var i = 0; i < blocks.length; i++) {
