...
export function AESEncrypt(message: string, key: string): string {
  ...
  var messageBytes = string2blocks(message, 16);
  let keyBytes = hex2ByteArray(key);
  let kSched = keySchedule(keyBytes, 11);
  let encrypted: string = "";

  for (var i = 0; i < messageBytes.length; i += 16) {
    encrypted += Array.from(
      encryptBlock(messageBytes.slice(i, i + 16), kSched, 11)
    )
...
export function AESDecrypt(ciphertext: string, key: string): string {
  ...
  for (var i = 0; i < byteArray.length; i += 16) {
    decrypted += decoder.decode(
      decryptBlock(byteArray.slice(i, i + 16), kSched, 11)
    );
  }
  return decrypted;
}
